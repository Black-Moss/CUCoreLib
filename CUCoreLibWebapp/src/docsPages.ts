...
export function pageBody(page: PageId, nextItemState: ItemState, nextRecipeState: RecipeState, nextIngredients: Ingredient[]): string {
  ...
  if (page === "unity-csharp") return unityCsharpPage();
  if (page === "setup") return setupPage();
  if (page === "harmony0") return harmony0Page();
  if (page === "recipe") return recipePage();
  if (page === "assets") return assetPage();
  if (page === "audio") return audioPage();
  if (page === "console") return consolePage();
  if (page === "utils") return utilsPage();
  if (page === "tools") return toolsPage();
  if (page === "item") return itemPage();
  if (page === "advanced-item") return advancedItemPage();
  if (page === "liquids") return liquidsPage();
  if (page === "player") return playerPage();
  if (page === "statuses") return statusesPage();
  if (page === "moodles") return moodlesPage();
  if (page === "building-entities") return buildingEntitiesPage();
  if (page === "advanced-building-entities") return advancedBuildingEntitiesPage();
  if (page === "minigames") return minigamesPage();
  if (page === "tiles") return tilesPage();
  if (page === "traps") return trapsPage();
  if (page === "locale") return localePage();
  if (page === "multi-mod-compatibility") return multiplayerPage();
  if (page === "settings") return settingsPage();
  if (page === "saving") return savingPage();
...
function localePage(): string {
  return `
    <section class="lesson-card">
      ...
      <p>That means your mod can stay English-only in code, generate one baseline file, hand that file to a translator, and then ship a matching locale file like <span class="inline-code">CN-zh.json</span> for automatic runtime replacement.</p>
